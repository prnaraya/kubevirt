...
var _ = Describe("[sig-compute][virtctl] mass machine type transition", func() {
	...
	Context("for a single VM with outdated machine type", func() {
		...
		Context("when VM machine type is less than minimum supported version", func() {
			...
			AfterEach(func() {
				...
				err = virtClient.BatchV1().Jobs("kubevirt").Delete(context.Background(), convertMachineType, metav1.DeleteOptions{})
				Expect(err).ToNot(HaveOccurred())
			})
