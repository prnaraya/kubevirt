...
`
	...
	Context("[rfe_id:273]with oc/kubectl", func() {
		...
		Context("Deleting a running VM with high TerminationGracePeriod via command line with", func() {
			It("--force --grace-period=0 should force delete the VM", func() {
			...
			It("--now should signal immediate shutdown for the VM", func() {
				By("getting a VM with a high TerminationGracePeriod")
				vmi := libvmi.New(
					libvmi.WithResourceMemory("128Mi"),
					libvmi.WithTerminationGracePeriod(1600),
